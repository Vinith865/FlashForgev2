...
let lastShowcaseWrite = { items: null, at: 0 };

export async function readShowcase() {
  if (lastShowcaseWrite.items && Date.now() - lastShowcaseWrite.at < WRITE_CACHE_MS) {
    return lastShowcaseWrite.items;
  }

  if (!usingBlob()) {
    try {
      const parsed = JSON.parse(await fs.readFile(LOCAL_SHOWCASE, 'utf8'));
      return Array.isArray(parsed) ? parsed : [];
    } catch {
      return [];
    }
  ...
  const url = await findBlobUrl(SHOWCASE_PATH);
  if (!url) return [];
  try {
    const res = await fetch(`${url}?t=${Date.now()}`, { cache: 'no-store' });
    if (!res.ok) return [];
...
export async function writeShowcase(items) {
  lastShowcaseWrite = { items, at: Date.now() };
  const payload = JSON.stringify(items, null, 2);

  if (!usingBlob()) {
    assertWritable();
    await fs.mkdir(path.dirname(LOCAL_SHOWCASE), { recursive: true });
    await fs.writeFile(LOCAL_SHOWCASE, payload, 'utf8');
    return items;
  }
  await putBlob(SHOWCASE_PATH, payload, 'application/json');
  return items;
}
