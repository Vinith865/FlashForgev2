...
import { FileCode2, Loader2, Plus, Trash2, Check } from 'lucide-react';
...
export default function FileManager({ project, onAdd, onSetOffset, onRemove, busy }) {
  ...
  const offsetValue = (file) =>
    drafts[file.filename] ?? Number(file.offset).toString(16);

  return (
    <div className="space-y-2">
      <ul className="space-y-2">
        {files.map((file) => {
          const dirty = drafts[file.filename] !== undefined &&
          ...
          return (
            ...
            >
              ...
              <input
                ...
                onChange={(e) =>
                  setDrafts({ ...drafts, [file.filename]: e.target.value.replace(/[^0-9a-fA-F]/g, '') })
                }
              ...
              <button
                ...
                className="rounded p-1 text-slate-500 transition hover:text-rose-300 disabled:opacity-30"
              >
                <Trash2 size={13} />
              </button>
            </li>
